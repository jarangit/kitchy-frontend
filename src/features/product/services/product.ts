...
export const productApiService = {
  ...
  getProductsByStoreId: async (storeId: string) => {
    if (IS_DEMO_MODE) {
      ...
      return { data: { success: true, message: "ok", data } } as {
        data: ApiResponse<IMenu[]>;
      };
    }
    return await axiosClient.get<ApiResponse<IMenu[]>>(
  ...
  createProduct: async (data: CreateProductRequest) => {
    if (IS_DEMO_MODE) {
      ...
      return { data: { success: true, message: "ok", data: product } } as {
        data: ApiResponse<IMenu>;
      };
    }
  ...
  updateProduct: async (productId: string, data: UpdateProductRequest) => {
    ...
    return await axiosClient.patch<ApiResponse<IMenu>>(
      `/products/${productId}`,
      data,
    );
  },
